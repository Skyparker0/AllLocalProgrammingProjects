# -*- coding: utf-8 -*-
"""
Created on Thu Mar 11 08:00:28 2021

@author: batte
"""


###FAILED- NEVER GAVE RESULT

def pentagonal_number(n):
    return int(n * (3 * n - 1) / 2)

pentagonals = [pentagonal_number(i) for i in range(1, 100000)]

def is_special(j,k):
    Pj, Pk = pentagonal_number(j), pentagonal_number(k)
    if abs(Pj - Pk) in pentagonals and abs(Pj + Pk) in pentagonals:
        return (Pj, Pk, abs(Pj - Pk))
    return None

for j in range(1, 1000):
    for k in range(j, 1000):
        result = is_special(j,k)
        if result != None:
            print(result)
