# -*- coding: utf-8 -*-
"""
Created on Sat Feb 12 11:27:23 2022
...
class Board:
    ...
    def __init__(self,width,height):
        self.width = width
        self.height = height
        self.queue = [2]*4 + [random.choice([0,1]) for i in range(width*height - 4)]
        random.shuffle(self.queue)
        self.circleArray = np.array([[self.queue.pop() for y in range(height)] for x in range(width)])
    ...
    def drop(self,x,y):
        if y < 0:
            self.set_val(x, y+1, self.queue.pop())
        ...
        if self.anchorPoint == (tileX,tileY):
            ...
            if all(self.get_val(fourCorners[i][0],fourCorners[i][1]) == startVal for i in range(4)) and startVal != -1:
                ...
                for i in range(4):
                    ...
                    if self.get_val(fourCorners[i][0], fourCorners[i][1]) == 2:
                        greenKilled += 1
                    else:
                        othersKilled += 1
                ...
                self.queue = [2]*greenKilled + [random.choice([0,1]) for i in range(othersKilled)]
                random.shuffle(self.queue)
