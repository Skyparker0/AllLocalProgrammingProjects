...
class all_objects:
    ...
    def __init__(self, drawingSurface, numRobotsPerRocket, rocketWeightLimit, minePositions, rocketPositions, speed, carryOffset, mineResourses, miningSpeed, depositSpeed):
        ...
        self.speed = speed
        self.robotCarry = rocketWeightLimit / numRobotsPerRocket - carryOffset    #####ROUND
        self.mineResourses = mineResourses
        self.mineSpeed = miningSpeed
        self.depositSpeed = self.robotCarry//10     #####################Changed- now they dump
        ######
    ...
    def tick(self):
        ...
        for base in self.bases:
            base.draw(self.drawingSurface)
            totalResources += base.get_stored()
        for mine in self.mines:
            mine.draw(self.drawingSurface)
        for robot in self.robots:
            robot.draw(self.drawingSurface)
            
        return totalResources >= goalResources-1
...
class robot(point):
    ...
    def draw(self, surface):
        ...
        pygame.draw.line(surface, (255,255,0), self.round_pos(), self.goal.round_pos(), 1)
    ...
    def choose_goal(self):
        ...
        if self.cargo == self.capacity or max(mine.resources for mine in self.master.mines) == 0:    #robot is full or all mines are depleted, head to a base
            ...
            if not max(mine.resources for mine in self.master.mines) == 0 and not self.cargo == 0:    
...
def simulate(ticks, surface, robotsInRocket, rocketWLimit, minePos, rocketPos, speed, carryOffset, mineResourses, miningSpeed, depositSpeed):
    ...
    for x in range(ticks):
        ...
        for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if event.button == 5:
                        timeScale = min(timeScale + 1, 100)

                    elif event.button == 4:
                        timeScale = max(timeScale - 1, 1)
...
numRobots = list(range(1,10)) + list(range(10,101,10))
...
carryOffset = 5    # how much is subtracted from a robot's carry weight
mineResourses = 1000  # how many pounds held by a single mine
miningSpeed = 10    # how fast a robot mines from a mine per tick
depositSpeed = 2    #how fast a robot deposits into a rocket per tick


def random_setups():
    base_setups = []
    mine_setups = []
    for x in range(1,11,3): #How many different scenarios? 
        base_setups.append([(random.randint(0,800),random.randint(0,800)) for i in range(1)])#How many bases?
        mine_setups.append([(random.randint(0,800),random.randint(0,800)) for i in range(x)])#How many mines?
...
for nRobots in numRobots:
    ...
    for setupNum in range(len(base_setups)):
        tickMap.append(simulate(maxTicks, screen, nRobots, rocketWeightLimit, mine_setups[setupNum], base_setups[setupNum], speed, carryOffset, mineResourses, miningSpeed, depositSpeed))
    numTicks.append(tickMap)#(sum(tickMap)/len(tickMap))pygame.quit()
sys.exit()