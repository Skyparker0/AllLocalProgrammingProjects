...
# rocketPositions = [(100,100)]    #2 rockets at the edges of the grid
...
depositSpeed = 10    #how fast a robot deposits into a rocket per tick

def random_setups():
    base_setups = []
    mine_setups = []
    for x in range(1,11,3): #How many different scenarios? 
        base_setups.append([(random.randint(0,700),random.randint(0,700)) for i in range(1)])#How many bases?
        mine_setups.append([(random.randint(0,700),random.randint(0,700)) for i in range(x)])#How many mines?
...
clock.tick(0.2)
simulate(maxTicks, screen, robotsInRocket,rocketWeightLimit,mine_setups[2],base_setups[0],speed, carryOffset, mineResourses, miningSpeed, depositSpeed)
pygame.quit()
sys.exit()