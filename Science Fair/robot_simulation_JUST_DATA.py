...
class all_objects:
    ...
    def __init__(self, numRobotsPerRocket, rocketWeightLimit, minePositions, rocketPositions, speed, carryOffset, mineResourses, miningSpeed, depositSpeed):
        ...
        self.speed = speed
        self.robotCarry = rocketWeightLimit / numRobotsPerRocket - carryOffset ###May need to round
        self.mineResourses = mineResourses
        self.mineSpeed = miningSpeed
        self.depositSpeed = self.robotCarry//depositSpeed
        ######
    ...
    def tick(self):
        ...
        for mine in self.mines:
            resourcesInMines += mine.get_resources()
            
        
        
        return totalResources >= goalResources-1  and  resourcesInMines == 0
...
class base(point):
    ...
    def add_resources(self, ammount):
        '''Increases the value held by base by ammount'''
        self.stored += ammount
        return ammount
...
def simulate(ticks, robotsInRocket, rocketWLimit, minePos, rocketPos, speed, carryOffset, mineResourses, miningSpeed, depositSpeed):
    all_obs = all_objects(robotsInRocket,rocketWLimit,minePos,rocketPos, speed, carryOffset, mineResourses, miningSpeed, depositSpeed)
    tickNum = 0
    ...
    for x in range(ticks):
        ...
        if done:
            print("done" + str(robotsInRocket))
            return tickNum
        tickNum += 1
    print(robotsInRocket)
    return ticks
